...
### 19. Resumable Crawl Frontier (New Subsystem)
- **Location:** `src/crawl/frontier.rs`, `src/crawl/crawler.rs` (`Crawler::with_frontier`/`Crawler::resumable`), `PyCrawl.resumable()`
- **Problem:** The charter lists "Incremental crawling" as a core subsystem; `Crawler::crawl_async` held its pending-URL queue and visited set as bare in-process `Vec`/`HashSet` locals — a crash, restart, or scheduled re-run always started a fresh crawl from `start_url`, with no way to resume or persist state across process boundaries. This was also the prerequisite the still-open sitemap and pagination subsystems need (large seed lists need somewhere durable to land).
- **Status: RESOLVED.** Extracted a `Frontier` trait (`enqueue`/`dequeue`/`mark_visited`/`is_visited`/`pending_len`/`visited_len`) with two implementations: `MemoryFrontier`, a LIFO `Vec` + `HashSet` behind a `Mutex` that reproduces the crawler's original behavior exactly (still the default — `Crawler.frontier: None`), and `PersistentFrontier`, `sled`-backed so reopening the same path resumes a previous run's pending queue and visited set instead of restarting from `start_url`. `Crawler::resumable(start_url, session, path)` is the one-line entry point; `Crawler::with_frontier(...)` accepts any custom `Arc<dyn Frontier>`. The start URL is only auto-seeded into a frontier that's completely empty (`pending_len() == 0 && visited_len() == 0`) — a frontier already carrying state from a prior run is left untouched, continuing exactly where it left off. Exposed to Python as `Crawl.resumable(start_url, path, session=None)`. **Scaling caveat, documented on `PersistentFrontier`:** the pending queue is one serialized `Vec` rewritten whole per enqueue/dequeue (`O(n)`) — fine at the thousands-of-URLs scale this targets, but a frontier meant for millions of pending URLs would need a real on-disk queue instead. The visited set, by contrast, is a proper per-URL sled key and stays `O(1)`.
