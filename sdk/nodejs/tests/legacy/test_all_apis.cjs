...
const os = require('os');
const { Page, Session, Dataset, Crawl, Watch, ElementCollection, Element, Sitemap, Downloader } = require('../dist');
...
const ROUTES = {
  ...
  '/download.bin': [200, 'file-download-data', 'application/octet-stream'],
};
...
async function testDownloads(runner, base) {
  ...
  try {
    ...
    runner.check('Downloader.downloadToMemory() status 200', result.status === 200);
    runner.check('Downloader.downloadToMemory() body content', data.toString().includes('file-download-data'));
  } catch (e) {
    runner.check('Downloader.downloadToMemory()', false, e.message);
  }
  ...
  try {
    ...
    runner.check('Sitemap.listUrls() count === 1', urls.length === 1);
    runner.check('Sitemap.listUrls() loc matches', urls[0].loc.endsWith('/json'));
  } catch (e) {
    runner.check('Sitemap.listUrls()', false, e.message);
  }
}
