---
title: Architecture & Internals
description: Deep dive into Crawlingo's Rust-powered core, FFI boundaries, and Apache Arrow-based memory management.
---

import { Server, Network, Database, Cpu } from 'lucide-react';
import { Tab, Tabs } from 'fumadocs-ui/components/tabs';

# Architecture & Internals

Crawlingo is split into a core Rust library and native bindings targeting Python, Node.js, and Go. All heavy-lifting calculations are done directly inside Rust memory space.

---

## Lifecycle Sequence Pipeline

<div className="grid grid-cols-1 md:grid-cols-4 gap-4 my-6">
  <HoverBorderCard className="p-5 flex flex-col justify-between h-full min-h-[140px]">
    <div>
      <span className="text-[10px] font-mono text-gray-400 dark:text-gray-500 uppercase font-semibold">Step 1</span>
      <h4 className="text-sm font-semibold text-black dark:text-white mt-1 mb-2">SDK Client</h4>
      <div className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">User initiates a Session script in Python, JS, or Go.</div>
    </div>
  </HoverBorderCard>

  <HoverBorderCard className="p-5 flex flex-col justify-between h-full min-h-[140px]">
    <div>
      <span className="text-[10px] font-mono text-gray-400 dark:text-gray-500 uppercase font-semibold">Step 2</span>
      <h4 className="text-sm font-semibold text-black dark:text-white mt-1 mb-2">Rust FFI Layer</h4>
      <div className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">Memory pointers are shared directly across Maturin/NAPI-RS boundaries.</div>
    </div>
  </HoverBorderCard>

  <HoverBorderCard className="p-5 flex flex-col justify-between h-full min-h-[140px]">
    <div>
      <span className="text-[10px] font-mono text-gray-400 dark:text-gray-500 uppercase font-semibold">Step 3</span>
      <h4 className="text-sm font-semibold text-black dark:text-white mt-1 mb-2">Parsing Engine</h4>
      <div className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">lol-html builds DOM maps; Jaro-Winkler scoring repairs drifted elements.</div>
    </div>
  </HoverBorderCard>

  <HoverBorderCard className="p-5 flex flex-col justify-between h-full min-h-[140px]">
    <div>
      <span className="text-[10px] font-mono text-gray-400 dark:text-gray-500 uppercase font-semibold">Step 4</span>
      <h4 className="text-sm font-semibold text-black dark:text-white mt-1 mb-2">Arrow Output</h4>
      <div className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">Streams memory blocks out to CSV, Parquet, or JSON formats.</div>
    </div>
  </HoverBorderCard>
</div>

---

## Core Engine Modules

<div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
  <HoverBorderCard className="p-6">
    <div className="flex items-center gap-2 mb-3">
      <div className="w-8 h-8 rounded-lg bg-black dark:bg-white text-white dark:text-black flex items-center justify-center">
        <Server size={16} />
      </div>
      <h4 className="text-sm font-semibold text-black dark:text-white">Session</h4>
    </div>
    <div className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">Central config holder holding user-agent headers, session cookies, rate limit, timeout, proxy strings, and browser profile settings.</div>
  </HoverBorderCard>

  <HoverBorderCard className="p-6">
    <div className="flex items-center gap-2 mb-3">
      <div className="w-8 h-8 rounded-lg bg-black dark:bg-white text-white dark:text-black flex items-center justify-center">
        <Network size={16} />
      </div>
      <h4 className="text-sm font-semibold text-black dark:text-white">Fetcher Engine (wreq)</h4>
    </div>
    <div className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">Binds dynamic connection pools, handles custom DNS resolution via hickory-resolver, and rotating browser TLS/JA3 fingerprints.</div>
  </HoverBorderCard>

  <HoverBorderCard className="p-6">
    <div className="flex items-center gap-2 mb-3">
      <div className="w-8 h-8 rounded-lg bg-black dark:bg-white text-white dark:text-black flex items-center justify-center">
        <Cpu size={16} />
      </div>
      <h4 className="text-sm font-semibold text-black dark:text-white">Streaming Parser (lol_html)</h4>
    </div>
    <div className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">Fast streaming HTML parser from Cloudflare that processes bytes on the fly and constructs vector-indexed element structures.</div>
  </HoverBorderCard>

  <HoverBorderCard className="p-6">
    <div className="flex items-center gap-2 mb-3">
      <div className="w-8 h-8 rounded-lg bg-black dark:bg-white text-white dark:text-black flex items-center justify-center">
        <Database size={16} />
      </div>
      <h4 className="text-sm font-semibold text-black dark:text-white">Fuzzy Selector Matcher</h4>
    </div>
    <div className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">Computes character-level tag matching and attribute weights using Jaro-Winkler distances to solve selector drift issues.</div>
  </HoverBorderCard>
</div>

---

## Cross-Language Bindings

Crawlingo leverages modern bindings to run Rust-level speed while exposing native SDK features:

<div className="grid grid-cols-1 md:grid-cols-2 gap-6 my-6">
  <div className="p-6 border border-gray-200 dark:border-white/10 rounded-2xl bg-white dark:bg-zinc-950">
    <h3 className="text-sm font-semibold text-black dark:text-white mb-2">Python Bindings (PyO3)</h3>
    <div className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
      Exposed natively via the PyO3 crate and managed via Maturin. Data conversion operates directly on memory buffers using Apache Arrow, mapping Python Lists to raw C structs without serialization penalties.
    </div>
  </div>
  <div className="p-6 border border-gray-200 dark:border-white/10 rounded-2xl bg-white dark:bg-zinc-950">
    <h3 className="text-sm font-semibold text-black dark:text-white mb-2">Node.js Bindings (NAPI-RS)</h3>
    <div className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
      Exposed natively through Javascript promises mapping to internal Tokio threads inside the Rust core. Fully typed definitions allow typescript projects to check properties natively.
    </div>
  </div>
</div>

---

## Memory Optimization

To maintain a minimal footprint, datasets stream results as Apache Arrow record batches, bypassing JavaScript or Python object allocation until explicitly requested by the SDK client.

<Tabs items={['Python', 'Node.js']}>
  <Tab value="Python">

    ```python
    # Streams batches directly from Rust heap to local file systems
    dataset.build().to_parquet("output_stream.parquet")
    ```

  </Tab>
  <Tab value="Node.js">

    ```typescript
    // Streams batches directly from Rust heap to local file systems
    await dataset.build().then(r => r.toParquet("output_stream.parquet"));
    ```

  </Tab>
</Tabs>

<Callout type="info" title="Need help with the classes?">
  Review the complete method definitions in the [API Reference](/api-reference).
</Callout>
